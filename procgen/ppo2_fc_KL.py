...
from procgen.policy_new import build_policy_sample
try:
...
def learn(*, network, env, total_timesteps, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4, id = None,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    policy = build_policy_sample(env, network, **network_kwargs)
    ...
    if model_fn is None:
        from procgen.model_fc_KL import Model
        model_fn = Model
            ...
            for _ in range(noptepochs):
                ...
                for start in range(0, nbatch, nbatch_train):
                    ...
                    if  update*nbatch >= (checkpoint[check_index] * 1e6) and (not saved_key_checkpoints[check_index]):
                        slices = (arr[mbinds] for arr in (obs, returns, masks, values))
