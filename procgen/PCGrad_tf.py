...
class PCGrad(optimizer.Optimizer):
    ...
    def compute_gradients(self, loss, var_list=None,
        ...
        print(proj_grads_flatten.shape)
...
class PDTGrad(optimizer.Optimizer):
  

    def __init__(self, optimizer, use_locking=False, name="PDTGrad"):
        """optimizer: the optimizer being wrapped
        """
        super(PDTGrad, self).__init__(use_locking, name)
        self.optimizer = optimizer
    ...
    def compute_gradients(self, loss, var_list=None,
        ...
        def proj_grad(grad_task):
            ...
            grad_task = grad_task - tf.minimum(proj_direction, 0.) * grads_task[0]
            return grad_task
...
class PDTGrad_InDA(optimizer.Optimizer):

    def __init__(self, optimizer, use_locking=False, name="PDTGrad_InDA"):
        """optimizer: the optimizer being wrapped
        """
        super(PDTGrad_InDA, self).__init__(use_locking, name)
        self.optimizer = optimizer

    def compute_gradients(self, loss, grads_train, var_list=None,
                        gate_gradients=GATE_OP,
                        aggregation_method=None,
                        colocate_gradients_with_ops=False,
                        grad_loss=None):
        
        grads_distill = tf.concat([tf.reshape(grad, [-1,])
                            for grad in tf.gradients(loss, var_list)
                            if grad is not None], axis=0)
        print(len(var_list))
        print(grads_distill)
        #print(grads_train[0])
        #grads_train = tf.concat([grads_train[i] for i in range(grads_train.shape[0])
        #                    if grads_train[i] is not None], axis=0)
        print(grads_train)
        # Projection
        inner_product = tf.reduce_sum(grads_distill * grads_train)
        proj_direction = inner_product / tf.reduce_sum(grads_train * grads_train)
        proj_grads_flatten = grads_distill - tf.minimum(proj_direction, 0.) * grads_train
        ...
        for idx, var in enumerate(var_list):
            grad_shape = var.get_shape()
            flatten_dim = np.prod([grad_shape.dims[i].value for i in range(len(grad_shape.dims))])
            proj_grad = proj_grads_flatten[start_idx:start_idx+flatten_dim]
            proj_grad = tf.reshape(proj_grad, grad_shape)
            if len(proj_grads) < len(var_list):
                proj_grads.append(proj_grad)
            else:
