...
class Model(object):
    ...
    def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train, 
        ...
        params = [v for v in total_params if ('new_pi' in v.name) or ('new_vf' in v.name) or ('new_impala/layer_15' in v.name)] 
        new_params = [v for v in total_params if ('new_impala' in v.name) and ('layer_15' not in v.name)] #srat
        self.new_params =  sorted(new_params, key=lambda v: v.name)
        sample_params = [v for v in total_params if ('impala' in v.name) and ('new' not in v.name) and ('layer_15' not in v.name)]
        self.sample_params =  sorted(sample_params, key=lambda v: v.name)
        
        l2_weight = tf.reduce_sum([tf.nn.l2_loss(v) for v in total_params])
        ...
        self.reset_loss_names = ['l2_loss', 'pi_kl_loss','vf_loss', 'impala_network_size']
        self.reset_stats_list = [reset_loss, pi_kl_loss, vf_loss, l2_weight] #reset
