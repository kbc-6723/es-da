...
def constfn(val):
    ...
    return f
DIR_NAME = 'train_log'
def learn(*, network, env1,env2,env3,total_timesteps, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4, id = None,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    policy = build_policy(env1, network, **network_kwargs)
    ...
    runner1 = Runner(env=env1, model=model, nsteps=nsteps, gamma=gamma, lam=lam)
    runner2 = Runner(env=env2, model=model, nsteps=nsteps, gamma=gamma, lam=lam)
    runner3 = Runner(env=env3, model=model, nsteps=nsteps, gamma=gamma, lam=lam)
    ...
    for update in range(1, nupdates+1):
        ...
        now_reward = safemean([epinfo['r'] for epinfo in epinfos])
        if runner == runner1:
            count1 += 1 
            if now_reward > 33:
                runner = runner2
            ...
            if now_reward > 33:
                runner = runner3
                print("runner2,runner3")
            else:
                runner = runner1 
                print("runner2,runner1")
        elif runner == runner3:
            count3 += 1 
            if now_reward > 33:
                '''
