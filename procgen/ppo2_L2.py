...
def learn(*, network, env, total_timesteps, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4, id = None,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    if model_fn is None:
        from procgen.model_L2 import Model
        model_fn = Model
    ...
    checkpoint = list(range(0,202,10))
    check_index = 0
    ...
    for update in range(1, nupdates+1):
        ...
        if  update*nbatch >= (checkpoint[check_index] * 1e6) and logger.get_dir() and is_mpi_root and (not saved_key_checkpoints[check_index]):
            ...
            savepath = osp.join(checkdir, '%i'%(checkpoint[check_index]+100)) if id == None else osp.join(checkdir, id+'%i'%(checkpoint[check_index]+100))
