...
def build_random_impala_cnn(unscaled_images, depths=[16,32,32], **conv_kwargs):
    ...
    mask_vbox = tf.Variable(tf.zeros_like(out, dtype=bool), trainable=False)
    mask_shape = tf.shape(out)
    ...
    img  = tf.where(mask_vbox, x=tf.zeros_like(out), y=out)
    ...
    out = tf.where(mask_vbox, x=out, y=rand_img, name='randout')
