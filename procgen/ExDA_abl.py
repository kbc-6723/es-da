...
def main():
    ...
    parser.add_argument('--seqeuntial_aug', type=int, default = 1)
    ...
    if args.data_aug is not None:       
        ...
        if args.data_aug == 'color_jitter':
            ...
            aug_func1_1 = rad.ColorJitterLayer(batch_aug, p_rand = 1)
            aug_func1_2 = rad.RandomConv(batch_aug)
    ...
    if args.data_aug2 is not None:       
        ...
        if args.data_aug2 == 'color_jitter':
            aug_func2 = rad.ColorJitterLayer(batch_aug, p_rand = 1)
        elif args.data_aug2 == 'gray':
            aug_func2 = rad.Grayscale(batch_aug, p_rand = 1)
        elif args.data_aug2 == 'flip':
            aug_func2 = rad.Rand_Flip(batch_aug , p_rand = 1)    
        elif args.data_aug2 == 'rotate':
            aug_func2 = rad.Rand_Rotate(batch_aug, p_rand = 1)    
        elif args.data_aug2 == 'crop':
            aug_func2 = rad.Crop(batch_aug, p_rand = 1)
        elif args.data_aug2 == 'cutout':
            aug_func2 = rad.Cutout(batch_aug, p_rand = 1)
        elif args.data_aug2 == 'cutout_color':
            aug_func2 = rad.Cutout_Color(batch_aug, p_rand = 1)
        elif args.data_aug2 == 'random_conv':
            aug_func2 = rad.RandomConv(batch_aug)
        else:
            pass
    
    if args.data_aug2 is not None:
        nbatch = nbatch * 3
    elif args.data_aug is not None:
        nbatch = nbatch * 2
    else:
        pass
    ...
    obs_buffer1 = np.array([obs_buffer[x :: 64] for x in range(32)])
    obs_buffer1 = obs_buffer1.reshape(-1,64,64,3)
    
    obs_buffer2 = np.array([obs_buffer[x :: 64] for x in range(32,64)])
    ...
    if args.data_aug2 is not None:     
        if args.data_aug == 'color_jitter' or args.data_aug == 'gray':       
            for i  in range(run_step):
                aug_obs = np.concatenate((aug_obs, aug_func2.do_augmentation(obs_buffer[i*batch_aug:(i+1)*batch_aug])),axis = 0)
    ...
    for update in range(1, nupdates+1):
        ...
        if args.data_aug is not None:     
            if args.data_aug == 'rccj':
                for i  in range(run_step):
                    if i == 0:
                        aug_obs = np.concatenate((obs_buffer, aug_func1_1.do_augmentation(obs_buffer[i*batch_aug:(i+1)*batch_aug])),axis = 0)
                    elif i <= 14:
                        aug_obs = np.concatenate((aug_obs, aug_func1_1.do_augmentation(obs_buffer[i*batch_aug:(i+1)*batch_aug])),axis = 0)
                    else:
                        aug_obs = np.concatenate((aug_obs, aug_func1_2.do_augmentation(obs_buffer[i*batch_aug:(i+1)*batch_aug])),axis = 0)
            elif args.data_aug != 'color_jitter' or args.data_aug != 'gray': 
                for i  in range(run_step):
                    if i == 0:
                        aug_obs = np.concatenate((obs_buffer, aug_func.do_augmentation(obs_buffer[i*batch_aug:(i+1)*batch_aug])),axis = 0)
                    else:
