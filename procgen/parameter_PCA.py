...
def main():
    ...
    parser.add_argument('--distribution_mode', type=str, default='easy_plus', choices=["easy", "hard", "easy_plus", "hard_minus", "exploration", "memory", "extreme"])
    ...
    load_path = '/home/kbc/imp/procgen_ppo/checkpoints/try1_coinrun_ppo_level10_' + '10'
    load(load_path)
    ...
    for i in range(len(ppo_steps)) :
        load_path = '/home/kbc/imp/procgen_ppo/checkpoints/try1_coinrun_ppo_level10_' + str(ppo_steps[i])
        load(load_path)
        data_label.append(i)
        var = tf.trainable_variables('ppo2_model')
        var_value = np.array(sess.run(var))
        data = np.array([])
        for val in var_value:
            #print(np.ndarray.flatten(val))
            data = np.concatenate((data,np.ndarray.flatten(val)), axis = 0)
        dataset.append(data)    
    ...
    for i in range(len(a2c_steps)) :
        load_path = '/home/kbc/imp/procgen_ppo/checkpoints/try1_coinrun_a2c_level10_' + str(a2c_steps[i])
        load_a2c(load_path)
        data_label.append(i)
        var = tf.trainable_variables('a2c_model')
        var_value = np.array(sess.run(var))
        data = np.array([])
        for val in var_value:
            #print(np.ndarray.flatten(val))
